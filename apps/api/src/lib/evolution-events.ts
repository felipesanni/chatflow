...
export async function processEvolutionEvent(app: FastifyInstance, params: ProcessEvolutionEventParams) {
  ...
  try {
    ...
    const resolvedRemoteJid = parsed.isGroup
      ? (parsed.remoteJid?.includes('@g.us')
          ? parsed.remoteJid
          : parsed.chatAliases.find((alias) => alias.includes('@g.us'))
            ?? (preMatchedOutboundMessage?.ticket.isGroup && preMatchedOutboundMessage.ticket.externalChatId.includes('@g.us')
              ? preMatchedOutboundMessage.ticket.externalChatId
              : null))
      : parsed.remoteJid ?? preMatchedOutboundMessage?.ticket.externalChatId;

    if (!resolvedRemoteJid) {
      if (parsed.isGroup) {
        app.log.warn({
          action: 'evolution_group_without_group_jid_ignored',
          event: parsed.event,
          instanceId: instance.id,
          externalMessageId: parsed.externalMessageId,
          remoteJid: parsed.remoteJid,
          chatAliases: parsed.chatAliases,
        }, 'Evento de grupo sem JID @g.us ignorado para evitar roteamento incorreto.');
      }
