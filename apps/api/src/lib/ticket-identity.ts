...
export function buildTicketAliasCandidates(params: {
  ...
  if (params.isGroup) {
    const groupJid = rawRemoteJid?.includes('@g.us')
      ? rawRemoteJid
      : aliases.find((alias) => alias.includes('@g.us')) ?? null;

    return Array.from(new Set([
      groupJid,
      canonicalChatId?.includes('@g.us') ? canonicalChatId : null,
