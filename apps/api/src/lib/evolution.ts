...
function pickGroupJid(values: unknown[]) {
  return pickNonEmptyStringList(values).find((value) => value.includes('@g.us')) ?? null;
}
...
function isEditProtocolType(value: unknown) {
  if (value === 14) {
    return true;
  }
...
export function parseEvolutionPayload(
  ...
  return {
    ...
    deletion,
    rawPayload: payload,
    isGroup: Boolean(groupJid ?? remoteJid?.includes('@g.us')),
    isEdited,
